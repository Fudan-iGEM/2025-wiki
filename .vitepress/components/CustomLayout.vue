...
const processContent = () => {
  nextTick(() => {
    ...
    try {
      ...
      if (citationRegex.test(html)) {
        hasChanges = true;
        html = html.replace(citationRegex, (match, refId) => {
          const refNumber = refId.substring(1); // 去掉 ^ 符号
          return `<sup class="citation-link"><a href="#ref-${refNumber}" onclick="highlightReference('ref-${refNumber}')">${refNumber}</a></sup>`;
        });
      }

      // 6. 处理文献列表 - 查找 ## Reference 标题下的内容
      const referenceRegex = /<h2[^>]*>Reference<\/h2>([\s\S]*?)(?=<h2|$)/i;
      const referenceMatch = html.match(referenceRegex);
      
      if (referenceMatch) {
        hasChanges = true;
        let referenceSection = referenceMatch[1];
        
        // 处理脚注格式的文献 [^1]: 内容
        const footnoteRegex = /\[(\^[0-9]+)\]:\s*(.*?)(?=\[|$)/gs;
        referenceSection = referenceSection.replace(footnoteRegex, (match, refId, content) => {
          const refNumber = refId.substring(1); // 去掉 ^ 符号
          return `<div class="reference-item" id="ref-${refNumber}">
            <span class="reference-number">[${refNumber}]</span>
            <span class="reference-content">${content.trim()}</span>
          </div>`;
        });
        
        // 替换原来的Reference部分
        html = html.replace(referenceRegex, `<h2>Reference</h2><div class="references-container">${referenceSection}</div>`);
      }
      ...
      if (hasChanges) {
        ...
        // 添加引用高亮功能的全局函数
        if (!(window as any).highlightReference) {
          (window as any).highlightReference = (refId: string) => {
            // 移除之前的高亮
            const previousHighlight = document.querySelector('.reference-highlighted');
            if (previousHighlight) {
              previousHighlight.classList.remove('reference-highlighted');
            }
            
            // 高亮目标文献
            const targetRef = document.getElementById(refId);
            if (targetRef) {
              targetRef.classList.add('reference-highlighted');
              ...
              // 3秒后移除高亮
              setTimeout(() => {
                targetRef.classList.remove('reference-highlighted');
              }, 3000);
            }
          };
        }
...
.citation-link a {
  ...
  padding: 2px 4px;
  border-radius: 3px;
  background: linear-gradient(135deg, rgba(0, 152, 161, 0.1), rgba(0, 188, 212, 0.1));
  border: 1px solid rgba(0, 152, 161, 0.2);
...
.citation-link a:hover {
  color: #ffffff;
  background: linear-gradient(135deg, #008794, #0e9f99);
  border-color: #008794;
  transform: translateY(-1px);
  box-shadow: 0 4px 8px rgba(0, 152, 161, 0.3);
}
...
.reference-item {
  ...
  border-radius: 12px;
  border: 1px solid rgba(0, 152, 161, 0.05);
  transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
  position: relative;
  backdrop-filter: blur(10px);
}
...
.reference-item:hover {
  ...
  border-color: rgba(0, 152, 161, 0.15);
  transform: translateY(-2px);
  box-shadow: 0 8px 25px rgba(0, 152, 161, 0.1);
}
...
.reference-content a {
  color: #0098a1;
  text-decoration: none;
  border-bottom: 1px solid rgba(0, 152, 161, 0.3);
  transition: all 0.3s ease;
}

.reference-content a:hover {
  color: #008794;
  border-bottom-color: #008794;
  background: rgba(0, 152, 161, 0.05);
  padding: 2px 4px;
  border-radius: 4px;
}
...
.reference-highlighted {
  ...
  background: linear-gradient(135deg, rgba(0, 188, 212, 0.15), rgba(85, 194, 187, 0.15)) !important;
  border-color: rgba(0, 188, 212, 0.4) !important;
  transform: translateY(-4px) !important;
  box-shadow: 
    0 12px 30px rgba(0, 152, 161, 0.2),
    0 0 0 3px rgba(0, 188, 212, 0.1) !important;
}

@keyframes highlight-pulse {
  0% {
    box-shadow: 
      0 8px 25px rgba(0, 152, 161, 0.1),
